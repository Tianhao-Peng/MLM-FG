...
class LightningModule(pl.LightningModule):
    ...
    def __init__(self, config, vocab, graph_vocab, dataset_length, normalizer, trainer):
        ...
        ## transformer
        if config.model_arch == 'CrossAttentionTail':
            self.blocks = CrossAttentionTail(config)
        elif config.model_arch == 'CrossAttentionEncoderBuilder':
            cross_attention_builder = cross_attention_model_builder.from_kwargs(
                ...
                activation='gelu',
                # hyper_mode=config.hyper_mode
                )
            self.blocks = cross_attention_builder.get()
        elif config.model_arch == 'CrossAttentionLinearRotary':
            if config.attention_type == 'linear_attention_cross':
                cross_attention_builder = CrossAttentionLinearRotary.from_kwargs(
                    n_layers=config.n_layer,
                    n_heads=config.n_head,
                    query_dimensions=config.n_embd//config.n_head,
                    value_dimensions=config.n_embd//config.n_head,
                    dropout=config.q_dropout,
                    feed_forward_dimensions=config.n_embd,
                    attention_type='linear_attention_cross',
                    feature_map=partial(GeneralizedRandomFeatures, n_dims=config.num_feats),
                    activation='gelu',
                    )
            self.blocks = cross_attention_builder.get()
        elif config.model_arch == 'SMILESonly':
            if config.attention_type == 'linear':
                smiles_builder = TransformerEncoderBuilder.from_kwargs(
                    n_layers=config.n_layer,
                    n_heads=config.n_head,
                    query_dimensions=config.n_embd//config.n_head,
                    value_dimensions=config.n_embd//config.n_head,
            ...
            elif config.attention_type == 'full_attention_smiles':
                smiles_builder = smiles_model_builder.from_kwargs(
                    n_layers=config.n_layer,
                    n_heads=config.n_head,
                    query_dimensions=config.n_embd//config.n_head,
                    value_dimensions=config.n_embd//config.n_head,
                    dropout=config.q_dropout,
                    feed_forward_dimensions=config.n_embd,
                    attention_type='full_attention_smiles',
                    feature_map=partial(GeneralizedRandomFeatures, n_dims=config.num_feats),
                    activation='gelu',
                    )
        ...
        elif config.model_arch == 'Graphonly':
            if config.attention_type == 'linear':
                twoD_builder = TransformerEncoderBuilder.from_kwargs(
                    n_layers=config.n_layer,
                    n_heads=config.n_head,
                    query_dimensions=config.n_embd//config.n_head,
                    value_dimensions=config.n_embd//config.n_head,
                    dropout=config.q_dropout,
                    feed_forward_dimensions=config.n_embd,
                    attention_type='linear', # full_modified linear_modified
                    feature_map=partial(GeneralizedRandomFeatures, n_dims=config.num_feats),
                    activation='gelu',
                    num_heads=config.n_head,
                    )
            elif config.attention_type == 'full_attention_spatial':
                twoD_builder = twoD_model_builder.from_kwargs(
                    n_layers=config.n_layer,
                    n_heads=config.n_head,
                    query_dimensions=config.n_embd//config.n_head,
                    value_dimensions=config.n_embd//config.n_head,
                    dropout=config.q_dropout,
                    feed_forward_dimensions=config.n_embd,
                    attention_type='full_attention_spatial', # full_modified linear_modified
                    feature_map=partial(GeneralizedRandomFeatures, n_dims=config.num_feats),
                    activation='gelu',
                    num_heads=config.n_head,
                    )
            elif config.attention_type == 'rotary_attention_spatial':
                twoD_builder = rotate_builder.from_kwargs(
                    n_layers=config.n_layer,
                    n_heads=config.n_head,
                    query_dimensions=config.n_embd//config.n_head,
                    value_dimensions=config.n_embd//config.n_head,
            ...
            self.blocks = twoD_builder.get()
        elif config.model_arch == 'MolFormer':
        ...
        # self.hypergraph_pos_encoding = GraphPositionalEncoding(d_model=config.n_embd, pos_type='hyper_laplacian')
            
        # input embedding stem
        self.pos_emb = None
        self.smiles_tok_emb = nn.Embedding(n_vocab+10, config.n_embd)
        # self.smiles_tok_emb = nn.Embedding(524, config.n_embd)
        n_graph_vocab = len(graph_vocab.keys())  # 123
        # print('n_graph_vocab:{p1}, n_vocab:{p2}'.format(p1=n_graph_vocab, p2=n_vocab))
        self.graph_tok_emb = nn.Embedding(n_graph_vocab+10, config.n_embd)
        # self.graph_tok_emb = nn.Embedding(133, config.n_embd)
        ...
        if config.task == 'classification':
            self.pred_head = self.Net(
                config.n_embd, len(config.target), dropout=config.dropout,
            )
            self.criterion = nn.BCELoss() #nn.CrossEntropyLoss()# nn.CrossEntropyLoss() | nn.BCEWithLogitsLoss(reduction='none')
        ...
        self.prediction_path = os.path.join(log_path, "prediction.pkl")
    ...
    def configure_optimizers(self):
        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                ...
                if pn.endswith('bias') or ('batch_norm' in pn and pn.endswith('weight')):
                    # biases and batch norm weights will not be decayed
                    no_decay.add(fpn)
                ...
                if pn.endswith('bias') or 'att_' in pn:
                    # all biases and attention weights will not be decayed
                    no_decay.add(fpn)
                elif pn.endswith('weight') and isinstance(m, whitelist_weight_modules):
                    # weights of whitelist modules will be weight decayed
                    decay.add(fpn)
                    ...
                    # weights of blacklist modules will NOT be weight decayed
                    no_decay.add(fpn)
                elif 'conv' in mn and pn.endswith('weight'):
                    # add your logic for convolution weights if necessary
                    decay.add(fpn)
                ...
                #     print('p', pn, p)
                #     # print('m', m)
                #     print('p.named_parameters()', p.named_parameters())
                #     for kn, k in p.named_parameters():
                #         fpnk = '%s.%s.%s' % (mn, pn, kn)
                #         if kn.endswith('bias'):
                #             # all biases will not be decayed
                #             no_decay.add(fpnk)
                #         elif kn.endswith('weight') and isinstance(p, whitelist_weight_modules):
                #             # weights of whitelist modules will be weight decayed
                #             decay.add(fpnk)
                #         elif kn.endswith('weight') and isinstance(p, blacklist_weight_modules):
                #             # weights of blacklist modules will NOT be weight decayed
                #             no_decay.add(fpnk)
        ...
        # no_decay.add('graph_pos_encoding.spatial_pos_encoder.eps')
        
        # # GIN
        # no_decay.add('graph_pos_encoding.spatial_pos_encoder.gin2.eps')
        # no_decay.add('graph_pos_encoding.spatial_pos_encoder.gin1.eps')
        ...
        optim_groups = [
            ...
            {"params": [param_dict[pn] for pn in sorted(list(no_decay))], "weight_decay": 0.0001},
        ]
        ...
        # print('optimizer = optimizers.FusedLAMB(optim_groups, lr=learning_rate, betas=betas)')
        optimizer = AdamW(optim_groups, lr=learning_rate, betas=betas)
        print('optimizer = AdamW(optim_groups, lr=learning_rate, betas=betas)')
        
        scheduler = LambdaLR(optimizer, lr_lambda=lambda epoch: 0.95 ** epoch)
        return {"optimizer": optimizer, "lr_scheduler": scheduler, "monitor": "validation_loss"}
        
        # total_steps = self.dataset_length * 100
...
#         print('smiles_idx', smiles_idx.shape)
#         print('graph_idx', graph_idx.shape)
#         print('hyper_adj', hyper_adj.shape)
        ...
        if config.model_arch == 'CrossAttentionTail':
            # position encoding
            # smiles_emb = self.smiles_pos_encoding(x=smiles_emb)
            # graph_emb = self.graph_pos_encoding(x=graph_emb, pos=graph_pos)
            smiles_emb, graph_emb = self.drop(smiles_emb), self.drop(graph_emb)
            # forward
            smiles_out, graph_out = self.blocks(x1=smiles_emb, x2=graph_emb, pos1=smiles_pos, pos2=graph_pos, 
                                                hyper_adj2=hyper_adj, smiles_mask=smiles_mask, graph_mask=graph_mask)
            token_embeddings = torch.cat((smiles_out, graph_out), dim=1)
            mask = torch.cat((smiles_mask, graph_mask), dim=1)
        elif config.model_arch == 'CrossAttentionEncoderBuilder':
            # position encoding
            # smiles_emb = self.smiles_pos_encoding(x=smiles_emb)
            # graph_emb = self.graph_pos_encoding(x=graph_emb, pos=graph_adj)
            # print('graph_emb', graph_emb)
            smiles_emb, graph_emb = self.drop(smiles_emb), self.drop(graph_emb)
            # forward
            smiles_out, graph_out = self.blocks(x1=smiles_emb, x2=graph_emb, pos1=smiles_pos, pos2=graph_pos, hyper_adj2=hyper_adj)
            # average
            token_embeddings = torch.cat((smiles_out, graph_out), dim=1)
            mask = torch.cat((smiles_mask, graph_mask), dim=1)
            # <CLS>
            # output = torch.cat((smiles_out[:,0,:], graph_out[:,0,:]), dim=1)
            # pred = self.pred_head(output)
        elif config.model_arch == 'CrossAttentionLinearRotary':
            if self.train_config.attention_type == 'linear_attention_cross':
                graph_emb = self.graph_pos_encoding(x=graph_emb, pos=graph_adj)
                smiles_emb, graph_emb = self.drop(smiles_emb), self.drop(graph_emb)
                smiles_out, graph_out = self.blocks(x1=smiles_emb, x2=graph_emb, pos1=smiles_pos, pos2=graph_pos, hyper_adj2=hyper_adj, length_mask1=LM(smiles_mask.sum(-1)), length_mask2=LM(graph_mask.sum(-1)))
                # average
                token_embeddings = torch.cat((smiles_out, graph_out), dim=1)
                mask = torch.cat((smiles_mask, graph_mask), dim=1)
                # # separate
                # smiles_output = self.output_func(smiles_out, smiles_mask, flag) # batch_size, dim
                # print(flag, ' smiles_output', smiles_output.shape, smiles_output)
                # graph_output = self.output_func(graph_out, graph_mask, flag)
                # print(flag, ' graph_output', graph_output.shape, graph_output)
                # output = torch.cat((smiles_output, graph_output), dim=1)
                # print(flag, ' output', output.shape, output)
                # pred = self.pred_head(output)
                # return pred, labels
        elif config.model_arch in ['SMILESonly']:
            if self.train_config.attention_type == 'linear':
                smiles_emb = self.smiles_pos_encoding(x=smiles_emb)
            ...
            elif self.train_config.attention_type == 'full_attention_smiles':
                # smiles_emb = self.smiles_pos_encoding(x=smiles_emb)
                smiles_emb = self.drop(smiles_emb)
                smiles_output = self.blocks(smiles_emb, pos=smiles_pos, length_mask=LM(smiles_mask.sum(-1)))  # Shape: (batch_size, seq_len, head_num, hidden_dim)
            elif self.train_config.attention_type == 'full':
                smiles_emb = self.smiles_pos_encoding(x=smiles_emb)
                smiles_emb = self.drop(smiles_emb)
                smiles_output = self.blocks(smiles_emb, length_mask=LM(smiles_mask.sum(-1)))  # Shape: (batch_size, seq_len, head_num, hidden_dim)
            elif self.train_config.attention_type == 'rotary':
                # smiles_emb = self.smiles_pos_encoding(x=smiles_emb)
                smiles_emb = self.drop(smiles_emb)
                smiles_output = self.blocks(smiles_emb, length_mask=LM(smiles_mask.sum(-1)))  # Shape: (batch_size, seq_len, head_num, hidden_dim)
            token_embeddings = smiles_output
            mask = smiles_mask
        elif config.model_arch in ['Graphonly']:
            # print('graph_emb', graph_emb)
            # graph_emb = self.graph_pos_encoding(x=graph_emb, pos=graph_pos)
            # print('graph_emb', graph_emb)
            # sys.exit()
            # graph_emb = self.hypergraph_pos_encoding(x=graph_emb, pos=hyper_adj)
            # print('NO PE before forward')
            if self.train_config.attention_type == 'rotary_attention_spatial':
                graph_emb = self.drop(graph_emb)
                graph_out = self.blocks(graph_emb, length_mask=LM(graph_mask.sum(-1)))  # Shape: (batch_size, seq_len, head_num, hidden_dim)
            elif self.train_config.attention_type in ['full', 'linear']:
                graph_emb = self.graph_pos_encoding(x=graph_emb, pos=graph_adj, twoDinfo=(edge_index, edge_attr))
                graph_emb = self.drop(graph_emb)
                graph_out = self.blocks(graph_emb)#, length_mask=LM(graph_mask.sum(-1)))  # Shape: (batch_size, seq_len, head_num, hidden_dim)
            else:
                # graph_emb = self.graph_pos_encoding(x=graph_emb, pos=graph_pos)
                graph_emb = self.drop(graph_emb)
                graph_out = self.blocks(graph_emb, pos=graph_pos, hyper_adj=hyper_adj, length_mask=LM(graph_mask.sum(-1))) #
            token_embeddings = graph_out 
            mask = graph_mask
        elif config.model_arch in ['MolFormer']:
            smiles_emb = self.drop(smiles_emb)
            smiles_output = self.blocks(smiles_emb, length_mask=LM(smiles_mask.sum(-1)))  # , length_mask=LM(smiles_mask.sum(-1))# Shape: (batch_size, seq_len, head_num, hidden_dim)
            token_embeddings = smiles_output
            mask = smiles_mask
    ...
    def calculate_loss(self, pred, labels):
        mask = torch.where(labels==-1, 0., 1.).to(labels.device)
        pred = pred * mask
        labels = labels * mask
    ...
    def evaluate_epoch_end(self, outputs):
        ...
        preds = torch.cat([x["predictions"] for x in outputs])
        actuals = torch.cat([x["labels"] for x in outputs])
        masks = torch.where(actuals==-1, 0, 1).bool()
        ...
        for i in range(num_tasks):
            ...
            val_loss_task = self.criterion(preds_task.float(), actuals_task.float())
            acc = cal_accuracy_score_sigmoid(actuals_task.cpu().numpy(),preds_task.cpu().numpy())
            try:
            ...
            aucs.append(auc)
            accs.append(acc)
        measure_result = [np.mean(aucs), np.mean(accs)]# torch.mean(torch.tensor(aucs))
        avg_loss = torch.mean(torch.tensor(vals))
...
#             measure_result = [roc_auc, acc]
    ...
    def training_step(self, batch, batch_idx):
        ...
        pred, labels = self.one_step(batch, batch_idx, 'training_step')
        # print('pred', pred)
        ...
        print('training_step pred', pred)
        print('training_step labels', labels)
    ...
    def validation_step(self, batch, batch_idx, dataset_idx):
        ...
        pred, labels = self.one_step(batch, batch_idx, 'validation_step')
        loss = self.calculate_loss(pred, labels)
        # pred = self.get_final_pred(pred)
    ...
    def validation_epoch_end(self, outputs_all):
        ...
        for dataset_idx, outputs in enumerate(outputs_all):
            ...
            # self.log('{}_loss'.format(split_name), avg_loss)
            # self.log('{}_{}'.format(split_name, self.measure_name), measure_result)
        ...
        if self.min_loss['max_test_cls_his'] == tensorboard_logs['test_' + self.measure_name]:
            with open(self.prediction_path, 'wb') as f:
                pickle.dump(outputs_all, f)

        if self.min_loss['min_test_reg_his'] == tensorboard_logs['test_' + self.measure_name]:
            with open(self.prediction_path, 'wb') as f:
                pickle.dump(outputs_all, f)
        ...
        if (tensorboard_logs["validation_loss"] < self.min_loss["min_validation_loss"]):
            ...
            # if self.current_epoch>(self.train_config.unfreeze+5):
        ...
        # tensorboard_logs["min_validation_loss"] = self.min_loss["min_validation_loss"]
        # tensorboard_logs["min_test_loss"] = self.min_loss["min_test_loss"]
...
def generate_scaffolds(dataset, balanced, val_size, test_size, log_every_n=1000):
    scaffolds = {}
...
def scaffold_split(dataset, valid_size, test_size, balanced, seed=None, log_every_n=1000):
    ...
    scaffold_sets = generate_scaffolds(dataset, balanced=balanced, val_size=valid_size, test_size=test_size)
    ...
    print('valid_cutoff', valid_cutoff)
...
def read_smiles(data_path, target, task):
    ...
    df = df.loc[idx]
    measures = df[target_list].values.tolist()
    labels = [[-1.0 if np.isnan(x) else x for x in x_l] for x_l in measures]
    assert len(labels)==len(smiles_data), print(len(labels), len(smiles_data))
...
class MoleculeModule(pl.LightningDataModule):
    def __init__(self, 
        ...
        data_path, target_list, task, splitting, results_path, graph_padding, train_load
    ):
        ...
        # self.smiles_data, self.labels = read_smiles(data_path=self.data_path, target=self.target_list, task=self.task)
        ...
        for key, value in self.vocab_encoder.items():
            ...
            pos += 1
        # # add new word to the word bags
        # for smiles in self.smiles_data:
        #     if '\n' in smiles:
        #         print('carriage return in mol')
        #     raw_regex = self.regex.findall(smiles.strip('\n'))
        #     for word in raw_regex:
        #         if word not in self.char2id.keys():
        #             self.char2id[word] = pos+4
        #             self.id2char[pos+4] = word
        #             pos += 1
        # added_word_num = len(self.char2id) - (len(self.vocab_encoder.keys())+4)
        # if  added_word_num > 0:
        #     print('Add new words:{}'.format(added_word_num))
        ...
        # self.tokenizer = MolTranBertTokenizer('bert_vocab.txt', self.regex)

    def read_file(self, filename):
        with open(os.path.join(self.data_path, filename), 'r') as f:
            df = pd.read_csv(f)
            smiles, labels = df['smiles'].tolist(), df[self.target_list].tolist()
    ...
    def prepare_data(self):
        if self.splitting == 'molformer':
            ...
            #                f"{self.data_path}, {self.target_list}, {self.task}, {self.splitting}, prepare_data_auto_split")
    
    def prepare_data_manual(self):
        print('prepare_data_from_molformer\n'*10)
        train_smiles, train_labels = read_smiles(os.path.join(self.data_path, 'train.csv'), self.target_list, self.task)
        ...
        valid_smiles, valid_labels = read_smiles(os.path.join(self.data_path, 'valid.csv'), self.target_list, self.task)
        test_smiles,  test_labels  = read_smiles(os.path.join(self.data_path, 'test.csv'), self.target_list, self.task)
        ...
        train_dataset = MolTestDataset(dataset=train_smiles, labels=train_labels, data_path=self.data_path, 
                                       target=self.target_list, char2id=self.char2id, graph_char2id=self.graph_char2id, regex=self.regex, 
...
#         print('test_smiles:{}, positive ratio:{}'.format(len(test_labels), Counter(test_labels)))
        ...
        test_dataset = MolTestDataset(dataset=test_smiles, labels=test_labels, data_path=self.data_path, 
                                       target=self.target_list, char2id=self.char2id, graph_char2id=self.graph_char2id, regex=self.regex, 
                                       graph_padding=self.graph_padding)
#         train_dataset = PropertyPredictionDataset(dataset=train_smiles, labels=train_labels, data_path=self.data_path, target=self.target_list, tokenizer=self.tokenizer)
#         valid_dataset = PropertyPredictionDataset(dataset=valid_smiles, labels=valid_labels, data_path=self.data_path, target=self.target_list, tokenizer=self.tokenizer)
#         test_dataset = PropertyPredictionDataset(dataset=test_smiles, labels=test_labels, data_path=self.data_path, target=self.target_list, tokenizer=self.tokenizer)
...
#             else:
            ...
            # print('ckpt_path'.format(p1=ckpt_path))
...
class ModelCheckpointAtEpochEnd(pl.Callback):
    ...
    def on_epoch_end(self, trainer, pl_module):
        ...
        if (current_epoch - self.last_epoch) == 1:
            self.last_epoch = current_epoch
            print('on_epoch_end current_epoch:{}'.format(current_epoch))
...
#             filename = "epoch{p1}_loss{p2}.ckpt".format(
#                             p1=current_epoch, p2=loss)
#             ckpt_path = os.path.join(trainer.checkpoint_callback.dirpath, filename)
#             trainer.save_checkpoint(ckpt_path)
    ...
    for k, v in checkpoint['state_dict'].items():
        if k in model_dict:
            if model_dict[k].shape == v.shape:
                ...
                # raise ValueError(f"Mismatched Key: {k} | Model shape: {model_dict[k].shape} | Checkpoint shape: {v.shape}")
        else:
            mismatched_keys.append((k, None, v.shape))
            # print('mismatched_keys k:{}, v.shape:{}'.format(k, v.shape))
            # assert k.split('.')[0] in ['smiles_lang_model', 'graph_lang_model', 'lang_model']

        # elif k=='tok_emb.weight' and model_dict['smiles_tok_emb.weight'].shape == v.shape:
        #     pretrained_dict['smiles_tok_emb.weight'] = v
        # elif 'hyper_linear' in k:
        #     new_k = k.replace('hyper_linear', 'hyper_layer.hyper_linear')
...
def main(config):
    ...
    train_config = {'batch_size':config.n_batch, 'num_workers':config.num_workers, 'valid_size':config.valid_size, 'test_size':config.test_size, 'data_path':config.data_path, 'target_list':config.target, 'task':config.task, 'splitting':config.splitting, 'results_path':config.results_path, 'graph_padding':config.graph_padding, 'train_load':config.train_load}
    ...
    # print('checkpoint_callback', checkpoint_callback, checkpoint_callback.dirpath, checkpoint_callback.filename, checkpoint_callback.best_model_path)
    ...
    # try:
        # trainer.test(model, dataset_loader.get_test_dataloader())
        # # trainer.fit(model=model, train_dataloaders=dataset_loader.train_dataloader(), val_dataloaders=[dataset_loader.val_dataloader(), dataset_loader.get_test_dataloader()])
        # trainer.fit(model=model, datamodule=dataset_loader)
        # trainer.test(model, dataset_loader.get_test_dataloader())
    ...
    if config.debug is True:
        ...
        # remove_tree(cachefiles)
    # return model.min_loss['best_test_' + model.measure_name], model.min_loss['best_test_acc']
